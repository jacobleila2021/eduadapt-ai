...
# Word/LXML rejects NULL bytes and most control characters from PDF extraction.
_INVALID_XML_CHARS = re.compile(r"[\x00-\x08\x0b\x0c\x0e-\x1f\ud800-\udfff\ufeff]")


def sanitize_docx_text(text: Any) -> str:
    """Make text safe for python-docx / lxml (PDF uploads often include bad bytes)."""
    if text is None:
        return ""
...
def _safe_paragraph(doc: Document, text: Any, style: str | None = None) -> None:
    clean = sanitize_docx_text(text).strip()
    if not clean:
        return
    if style:
...
def _add_heading(doc: Document, text: str, level: int = 1) -> None:
    heading = doc.add_heading(sanitize_docx_text(text), level=min(level, 3))
...
def _add_colored_box(doc: Document, label: str, body: str, rgb: RGBColor) -> None:
    p = doc.add_paragraph()
    run = p.add_run(f"{sanitize_docx_text(label)}: ")
    run.bold = True
    run.font.color.rgb = rgb
    run.font.name = "Arial"
    run.font.size = Pt(14)
    safe_body = sanitize_docx_text(body)
    if safe_body:
        p.add_run(safe_body)
...
def _add_bullets(doc: Document, items: list) -> None:
    for item in items:
        _safe_paragraph(doc, item, style="List Bullet")
...
def export_vocabulary_docx(data: Any) -> bytes:
    ...
    for word in vocab.get("word_wall") or []:
        ...
        if visual:
            _safe_paragraph(doc, f"Picture: {visual}")
...
def export_worksheet_docx(data: Any, *, include_teacher_key: bool = False) -> bytes:
    ...
    _safe_paragraph(doc, "Name: _________________________    Date: _________________________")

    _add_heading(doc, "Part A — Short Answer", 2)
    for index, item in enumerate(sheet.get("short_answer") or [], 1):
        _safe_paragraph(doc, f"Q{index}. ({item.get('marks', 2)} marks) {item.get('question', '')}")
        for _ in range(int(item.get("lines", 3))):
            _safe_paragraph(doc, "_" * 70)

    _add_heading(doc, "Part B — Long Answer", 2)
    for index, item in enumerate(sheet.get("long_answer") or [], 1):
        _safe_paragraph(doc, f"Q{index}. ({item.get('marks', 6)} marks) {item.get('question', '')}")
    ...
    if diagram:
        ...
        _safe_paragraph(doc, f"({diagram.get('marks', 4)} marks) {diagram.get('question', '')}")
        _safe_paragraph(doc, "[Sketch and label the diagram on your answer sheet]")

    _add_heading(doc, "Part D — Vocabulary", 2)
    for index, item in enumerate(sheet.get("vocab_questions") or [], 1):
        _safe_paragraph(doc, f"{index}. ({item.get('marks', 1)} mark) {item.get('question', '')}")
        _safe_paragraph(doc, "Answer: _________________________________")
...
def export_tab_docx(title: str, content: Any, spec_id: str) -> bytes:
    ...
    if spec_id == "original":
        ...
        _apply_ld_normal_style(doc)
        _add_heading(doc, title)
        for para in sanitize_docx_text(str(content)).split("\n\n"):
            _safe_paragraph(doc, para)
        buffer = io.BytesIO()
        doc.save(buffer)
        return buffer.getvalue()
