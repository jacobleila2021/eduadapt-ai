"""Optional VLIE-compatible engine wrapper for CIP (not auto-registered in VLIE)."""
...
from engines.chemistry_intelligence.service import analyse_chemistry_lesson, pack_health
...
class ChemistryIntelligenceEngine(BaseEngine):
    """
    Opt-in Teaching-layer engine. Prefer SIF registry path via ULI pipeline.

    Does not replace Computation Layer chemistry balancer / RDKit / ChemPy.
    """

    engine_id = "chemistry_intelligence"
    version = "1.0.0"
    layer = "teaching"
    priority = 56
    ...
    def process(self, context: dict[str, Any]) -> EngineResultBundle:
        ...
        if uli is None:
            ...
            if not envelope:
                return EngineResultBundle(
                    ...
                    ok=False,
                    errors=["CIP requires universal_lesson_intelligence or source_envelope"],
                    deterministic=True,
                )
        ...
        analysis = analyse_chemistry_lesson(uli, context=context)
        return EngineResultBundle(
            engine_id=self.engine_id,
            ok=analysis.ok,
            payload={
                "chemistry_intelligence": analysis.to_dict(),
                "health": pack_health(),
            },
