"""Social science validation — additive SSIP signals; never invents content."""
...
def _ssip_quality_findings(uli: Any) -> list[ValidationFinding]:
    try:
        from engines.social_science_intelligence.validation import collect_social_science_quality_signals
    except Exception:  # noqa: BLE001
        return []
    try:
        signals = collect_social_science_quality_signals(uli)
    except Exception:  # noqa: BLE001
        return []
    ...
    for seed in signals.get("findings_seed") or []:
        ...
        out.append(
            finding(
                str(seed.get("rule_id") or "ULIQE.SOC.SSIP.999"),
                str(seed.get("category") or "pedagogy"),
                severity,
                str(seed.get("message") or "SSIP quality signal"),
                evidence=dict(seed.get("evidence") or {}) if isinstance(seed.get("evidence"), Mapping) else {},
                recommendation="Review social science pedagogy metadata; certification rules unchanged.",
            )
        )
    return out


def validate_social_science(uli: Any) -> list[ValidationFinding]:
    obj = coerce_uli(uli)
    blob_parts: list[str] = []
    try:
    ...
    if not applicable:
        ssip = _ssip_quality_findings(obj)
        base = [
            finding(
                "ULIQE.SOC.000",
                "stem_accuracy",
                FindingSeverity.INFO,
                "Social science validation skipped — not applicable.",
            )
        ]
        return base + ssip if ssip else base
    ...
    findings = [
        finding(
            ...
            "stem_accuracy",
            FindingSeverity.INFO,
            "Social science inventory checks completed (no invented content).",
        )
    ]
    findings.extend(_ssip_quality_findings(obj))
    return findings
