"""Optional VLIE-compatible engine wrapper for BIP (not auto-registered in VLIE)."""
...
from engines.biology_intelligence.service import analyse_biology_lesson, pack_health
...
class BiologyIntelligenceEngine(BaseEngine):
    """
    Opt-in Teaching-layer engine. Prefer SIF registry path via ULI pipeline.

    Does not invent biology facts or replace Knowledge Layer curriculum.
    """

    engine_id = "biology_intelligence"
    version = "1.0.0"
    layer = "teaching"
    priority = 56
    ...
    def process(self, context: dict[str, Any]) -> EngineResultBundle:
        ...
        if uli is None:
            ...
            if not envelope:
                return EngineResultBundle(
                    ...
                    ok=False,
                    errors=["BIP requires universal_lesson_intelligence or source_envelope"],
                    deterministic=True,
                )
        ...
        analysis = analyse_biology_lesson(uli, context=context)
        return EngineResultBundle(
            engine_id=self.engine_id,
            ok=analysis.ok,
            payload={
                "biology_intelligence": analysis.to_dict(),
                "health": pack_health(),
            },
