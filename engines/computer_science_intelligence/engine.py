"""Optional VLIE-compatible engine wrapper for CSIP (not auto-registered)."""
...
from engines.computer_science_intelligence.service import analyse_computer_science_lesson, pack_health
...
class ComputerScienceIntelligenceEngine(BaseEngine):
    engine_id = "computer_science_intelligence"
    version = "1.0.0"
    layer = "teaching"
    priority = 57
    ...
    def process(self, context: dict[str, Any]) -> EngineResultBundle:
        ...
        if uli is None:
            ...
            if not envelope:
                return EngineResultBundle(
                    ...
                    ok=False,
                    errors=["CSIP requires universal_lesson_intelligence or source_envelope"],
                    deterministic=True,
                )
        ...
        analysis = analyse_computer_science_lesson(uli, context=context)
        return EngineResultBundle(
            engine_id=self.engine_id,
            ok=analysis.ok,
            payload={"computer_science_intelligence": analysis.to_dict(), "health": pack_health()},
